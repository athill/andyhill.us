...
class TemplateInstance {
	...
	public function __construct($base) {
		...
		$this->scripts = array_merge($this->scripts, array("/js/site.js"));
	}
...
//		$array = $this->base->menu->xmlMenu2array($this->xml);
	...
	private function displaySearch() {
		...
		$sitesearch = array("|Web", $_SERVER['HTTP_HOST'].$webroot."|Site");
/*
...
*/
		$h->tnl("Search ");
		$h->select("sitesearch", $sitesearch, $_SERVER['HTTP_HOST']."$webroot");
...
//	  $h->pa($breadcrumbs);
	...
	public function footer() {
		...
		$h->otag('footer');
//		$h->tbr("footer");
		$h->tnl('&copy;'.$site['siteName'].'  ' . date('Y'));
		$h->ctag('footer');
		$h->cdiv();	////close page
		$h->chtml();
	}
