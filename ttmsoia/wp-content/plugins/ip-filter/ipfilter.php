...
class IPFilter
{
	...
	public function options_page_callback()
	{
		...
		<div class="wrap">
			...
			<textarea readonly='readonly' cols='140' rows='15'><?php
	...
	public function admin_init_callback()
	{
		...
		add_settings_field('ipfilter_setting_denymessage'   , __( 'Message shown to filtered visitors:'                            , IPFILTER_TEXTDOMAIN ), array( &$this, 'options_display_denymessage_callback'   ), 'ipfilter_options_page_id', 'ipfilter_options_section_general' );
	...
	public function options_display_filtertype_callback()
	{
		...
		if( $options === FALSE || !isset( $options['filter_type'] ) || empty( $options['filter_type'] ) )
		{
			$options['filter_type'] = IPFILTER_DEFAULT_FILTER;
		}
		?>
		<input
			id='ipfilter_setting_filtertype1'
		...
		<input
			id='ipfilter_setting_filtertype2'
			name='ipfilter_options[filter_type]'
			type='radio'
	...
	public function options_display_filteredips_callback()
	{
		$options = $this->get_option( NULL );
		//set a default value if no value is set
		if( $options === FALSE || !isset( $options['filtered_ips'] ) || empty( $options['filtered_ips'] ) )
		{
			$options['filtered_ips'] = '';
		}
		?>
		<textarea id='ipfilter_setting_filteredips' name='ipfilter_options[filtered_ips]' cols='40' rows='12' style='display: inline-block'><?php echo $options['filtered_ips']; ?></textarea>
		<textarea cols='25' rows='12' readonly='readonly' style='display: inline-block'>
<?php _e( 'Extracted:', IPFILTER_TEXTDOMAIN ); ?>
<?php foreach( $this->filtered_ips as $ip ): echo "\n" . $ip; endforeach; ?>
		</textarea>
		<?php
	}
	...
	public function options_display_denymessage_callback()
	{
		...
		$options = $this->get_option( NULL );
		//set a default value if no value is set
		if( $options === FALSE || !isset( $options['deny_message'] ) || empty( $options['deny_message'] ) )
		{
			$options['deny_message'] = $ipfilter_default_deny_message;
		}
		?>

		<input id='ipfilter_setting_denymessage' name='ipfilter_options[deny_message]' size='60' type='text' value='<?php echo $options['deny_message']; ?>' />
	...
	public function options_display_logblockedips_callback()
	{
		$options = $this->get_option( NULL );
		//set a default value if no value is set
		if( $options === FALSE || !isset( $options['log_blocked_ips'] ) || empty( $options['log_blocked_ips'] ) )
		{
