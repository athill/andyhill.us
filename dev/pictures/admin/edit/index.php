...
////Select category
$h->oform("");
$h->select('category', array('family','nationalmall', 'texas'), $h->getVal('category'));
...
if (array_key_exists($category, $data1)) {
	$data = $data1[$category];
} else {
...
//		$h->odiv('style="border: thin solid black; padding: 1em;"');
		if ($i > 0) $h->corow();
		$item = (array_key_exists($i, $data)) ? $data[$i]: array();
		$h->otd();
		...
		$value = str_replace('\"', '"', getVal('title', $item));
		$value = str_replace("\\'", "'", $value);
		$h->intext($name.'_title', $value);
		$h->br();
		$h->label($name.'_description', 'description: ');
		$h->br();
		$value = str_replace('\"', '"', getVal('description', $item));
		$value = str_replace("\\'", "'", $value);
		$h->textarea($name.'_description', $value);
		$h->br();
